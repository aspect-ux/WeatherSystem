	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				float noise = tex2D(_CloudNoise, ((skyuv ) - (_Time.x * _CloudSpeed)) * _CloudNoiseScale);
				...
				horizon = saturate((1 - horizon)) * ((_HorizonColorDay + midline * _HorizonLightDay) *
					saturate(_WorldSpaceLightPos0.y*10)
					+ (_HorizonColorNight + midline * _HorizonLightNight) * saturate(-_WorldSpaceLightPos0.y * 10))
					* _HorizonBrightness;
