...
public class LightningSystem : MonoBehaviour {
	...
	void Start () {
		GameObject TempBolt = Resources.Load("Prefabs/Lightning Renderer") as GameObject;
		LightningBolt = Instantiate(TempBolt, Vector3.zero, Quaternion.identity).GetComponent<LineRenderer>();
		LightningBolt.transform.SetParent(GameObject.Find("Wendy Weather System").transform);
		LightningBolt.name = "Lightning Renderer";

		GameObject TempEndPoint = Resources.Load("Prefabs/Lightning End Point") as GameObject;
		EndingPoint = Instantiate(TempEndPoint, Vector3.zero, Quaternion.identity).transform;
		EndingPoint.transform.SetParent(GameObject.Find("Wendy Weather System").transform);
		EndingPoint.name = "Lightning End Point";

		StartingPoint = new GameObject().transform;
		StartingPoint.transform.position = Vector3.zero;
		StartingPoint.SetParent(GameObject.Find("Wendy Weather System").transform);
		...
		AS.outputAudioMixerGroup = FindObjectOfType<WeatherSystem>().wendyAudioMixer.FindMatchingGroups("Master/Weather")[0];
