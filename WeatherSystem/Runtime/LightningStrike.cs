...
public class LightningStrike : MonoBehaviour {
	...
	void Start ()
	{
		WeatherSystem WeatherSystemTemp = GameObject.Find("Wendy Weather System").GetComponent<WeatherSystem>();
		WeatherSystemTemp.m_LightningStrikeSystem = GetComponent<LightningStrike>();
		GroundStrikeOdds = WeatherSystemTemp.LightningGroundStrikeOdds;
		LightningStrikeEffect = WeatherSystemTemp.LightningStrikeEffect;
		LightningStrikeFire = WeatherSystemTemp.LightningStrikeFire;
		DetectionLayerMask = WeatherSystemTemp.DetectionLayerMask;
		LightningFireTags = WeatherSystemTemp.LightningFireTags;
		GetComponent<SphereCollider>().radius = WeatherSystemTemp.LightningDetectionDistance;
	...
	public void CreateLightningStrike()
	{
		...
		if (Physics.Raycast(new Vector3(HitPosition.x, HitPosition.y+40, HitPosition.z), -transform.up, out hit, RaycastDistance, DetectionLayerMask))
		{
			...
			if (!PlayerDetected)
			{
				...
				{
					GameObject HitEffect = WendyPool.Spawn(LightningStrikeFire, HitPosition, Quaternion.identity);
					HitEffect.transform.SetParent(hit.collider.transform);
				}

				WendyPool.Spawn(LightningStrikeEffect, HitPosition, Quaternion.identity);
			}
			else if (PlayerDetected)
			{
				if (LightningFireTags.Contains(hit.collider.tag))
				{
					WendyPool.Spawn(LightningStrikeFire, HitPosition, Quaternion.identity);
				}
